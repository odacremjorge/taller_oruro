...
def admin_report_view(request):
    reports=models.Request.objects.all().filter(Q(status="Reparacion Finalizada") | Q(status="Liberado"))
    dict={
        'reports':reports,
    }
...
def customer_dashboard_view(request):
    ...
    work_in_progress=models.Request.objects.all().filter(customer_id=customer.id,status='Reparando').count()
    work_completed=models.Request.objects.all().filter(customer_id=customer.id).filter(Q(status="Reparacion Finalizada") | Q(status="Liberado")).count()
    ...
    bill=models.Request.objects.all().filter(customer_id=customer.id).filter(Q(status="Reparacion Finalizada") | Q(status="Liberado")).aggregate(Sum('cost'))
    print(bill)
    dict={
...
def mechanic_dashboard_view(request):
    ...
    work_in_progress=models.Request.objects.all().filter(mechanic_id=mechanic.id,status='Reparando').count()
    work_completed=models.Request.objects.all().filter(mechanic_id=mechanic.id,status='Reparacion Finalizada').count()
...
def mechanic_salary_view(request):
    mechanic=models.Mechanic.objects.get(user_id=request.user.id)
    workdone=models.Request.objects.all().filter(mechanic_id=mechanic.id).filter(Q(status="Reparacion Finalizada") | Q(status="Liberado"))
